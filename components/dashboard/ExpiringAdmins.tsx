...
export function ExpiringAdmins({ admins, loading = false }: ExpiringAdminsProps) {
  if (loading) {
    return (
      <Card>
        <CardHeader>
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-col items-center justify-center py-8 text-center">
            <Users className="h-12 w-12 text-muted-foreground mb-4" weight="light" />
            <p className="text-sm text-muted-foreground">No admins expiring soon</p>
          </div>
        </CardContent>
      </Card>
    )
  ...
  const getValidityColor = (days: number | undefined) => {
    if (!days) return 'bg-gray-500/10 text-gray-700 dark:text-gray-400 border-gray-500/20'
    if (days <= 7) return 'bg-red-500/10 text-red-700 dark:text-red-400 border-red-500/20'
    if (days <= 30) return 'bg-yellow-500/10 text-yellow-700 dark:text-yellow-400 border-yellow-500/20'
      ...
      <CardContent className="space-y-4">
        {admins.map((admin) => (
          ...
          >
            ...
            <Badge variant="outline" className={getValidityColor(admin.adminValidity)}>
